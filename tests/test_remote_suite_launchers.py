...
class RemoteSuiteLauncherContractsTest(unittest.TestCase):
    def test_remote_leq2b_candidate_bundle_helper_builds_bundle_and_routes_from_lane_dirs(self):
        ...
        self.assertIn("ARG=--gate\nARG=promotion\n", log_text)
        self.assertIn(
            "ARG=--output\nARG=outputs/optimization_sweeps/leq2b-bundle-unit/leq2b.routes.json\n",
            log_text,
        )

    def test_remote_leq2b_candidate_bundle_helper_can_skip_route_export(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            log_file = tmp_path / "remote.log"
            fake_remote = tmp_path / "remote_exec.sh"
            ...
            result = subprocess.run(
                ...
                env=isolated_remote_env(
                    ...
                    FAKE_REMOTE_LOG=str(log_file),
                    JETSON_LEQ2B_BUNDLE_RUN_PREFIX="leq2b-bundle-unit",
                    JETSON_LEQ2B_BUNDLE_SELECTIONS="outputs/optimization_sweeps/light-unit/promotion.leq2b-vlm.selection.json",
                    JETSON_LEQ2B_BUILD_ROUTES="0",
                    JETSON_LEQ2B_ROUTE_GATE="not-a-gate",
                ),
            )
        ...
        self.assertEqual(log_text.count("CALL\n"), 1)
        self.assertIn("ARG=bundle-selections\n", log_text)
        self.assertNotIn("ARG=export-routes\n", log_text)
    ...
    def test_remote_current_defaults_suite_runs_locked_sweep_then_compare(self):
        ...
        self.assertIn("REMOTE_ARG=--startup-require-cached-artifacts\n", log_text)
        self.assertIn("REMOTE_ARG=--ranking-min-lfb-blocks\nREMOTE_ARG=199\n", log_text)
        self.assertIn("REMOTE_ARG=--ranking-require-startup-precheck\n", log_text)
        self.assertIn("REMOTE_ARG=--promotion-precheck-stage\nREMOTE_ARG=promotion-reference\n", log_text)
        self.assertIn("REMOTE_ARG=--promotion-require-startup-precheck\n", log_text)
        self.assertIn("REMOTE_ARG=--promotion-require-quality-review\n", log_text)
        self.assertNotIn("REMOTE_ARG=--fail-on-ranking-precheck\n", log_text)
    ...
    def test_remote_current_defaults_suite_can_fail_on_startup_precheck_when_enabled(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            log_file = tmp_path / "suite.log"
            fake_sweep, fake_remote = write_fake_suite_scripts(tmp_path)
            ...
            result = subprocess.run(
                ...
                encoding="utf-8",
                env=isolated_remote_env(
                    JETSON_CURRENT_DEFAULTS_RUN_PREFIX="defaults-startup-fail-unit",
                    JETSON_CURRENT_DEFAULTS_FAIL_ON_STARTUP_PRECHECK="1",
                    JETSON_REMOTE_SYNC="0",
                    JETSON_REMOTE_SWEEP=str(fake_sweep),
                    JETSON_REMOTE_EXEC=str(fake_remote),
                    FAKE_SUITE_LOG=str(log_file),
        ...
        self.assertIn("REMOTE_ARG=--startup-require-cached-artifacts\n", log_text)
        self.assertIn("REMOTE_ARG=--fail-on-startup-precheck\n", log_text)

    def test_remote_current_defaults_suite_can_fail_on_ranking_precheck_when_enabled(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            log_file = tmp_path / "suite.log"
            fake_sweep, fake_remote = write_fake_suite_scripts(tmp_path)
            ...
            result = subprocess.run(
                ...
                encoding="utf-8",
                env=isolated_remote_env(
                    JETSON_CURRENT_DEFAULTS_RUN_PREFIX="defaults-ranking-fail-unit",
                    JETSON_CURRENT_DEFAULTS_FAIL_ON_RANKING_PRECHECK="1",
                    JETSON_REMOTE_SYNC="0",
                    JETSON_REMOTE_SWEEP=str(fake_sweep),
                    JETSON_REMOTE_EXEC=str(fake_remote),
                    FAKE_SUITE_LOG=str(log_file),
        ...
        self.assertIn("REMOTE_ARG=--ranking-require-startup-precheck\n", log_text)
        self.assertIn("REMOTE_ARG=--fail-on-ranking-precheck\n", log_text)
    ...
    def test_remote_lightweight_model_suite_can_fail_on_startup_precheck_when_enabled(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            log_file = tmp_path / "suite.log"
            fake_sweep, fake_remote = write_fake_suite_scripts(tmp_path)
            ...
            result = subprocess.run(
                ...
                encoding="utf-8",
                env=isolated_remote_env(
                    JETSON_LIGHTWEIGHT_RUN_PREFIX="light-startup-fail-unit",
                    JETSON_LIGHTWEIGHT_FAIL_ON_STARTUP_PRECHECK="1",
                    JETSON_REMOTE_SYNC="0",
                    JETSON_REMOTE_SWEEP=str(fake_sweep),
                    JETSON_REMOTE_EXEC=str(fake_remote),
                    FAKE_SUITE_LOG=str(log_file),
    ...
    def test_remote_lightweight_model_suite_can_fail_on_ranking_precheck_when_enabled(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            log_file = tmp_path / "suite.log"
            fake_sweep, fake_remote = write_fake_suite_scripts(tmp_path)
            ...
            result = subprocess.run(
                ...
                encoding="utf-8",
                env=isolated_remote_env(
                    JETSON_LIGHTWEIGHT_RUN_PREFIX="light-ranking-fail-unit",
                    JETSON_LIGHTWEIGHT_FAIL_ON_RANKING_PRECHECK="1",
                    JETSON_REMOTE_SYNC="0",
                    JETSON_REMOTE_SWEEP=str(fake_sweep),
                    JETSON_REMOTE_EXEC=str(fake_remote),
                    FAKE_SUITE_LOG=str(log_file),
    ...
    def test_remote_tencent_text_suite_can_fail_on_startup_precheck_when_enabled(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            log_file = tmp_path / "suite.log"
            fake_sweep, fake_remote = write_fake_suite_scripts(tmp_path)
            ...
            result = subprocess.run(
                ...
                encoding="utf-8",
                env=isolated_remote_env(
                    JETSON_TENCENT_TEXT_RUN_PREFIX="tencent-text-startup-fail-unit",
                    JETSON_TENCENT_TEXT_FAIL_ON_STARTUP_PRECHECK="1",
                    JETSON_REMOTE_SYNC="0",
                    JETSON_REMOTE_SWEEP=str(fake_sweep),
                    JETSON_REMOTE_EXEC=str(fake_remote),
                    FAKE_SUITE_LOG=str(log_file),
    ...
    def test_remote_tencent_text_suite_can_fail_on_ranking_precheck_when_enabled(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            log_file = tmp_path / "suite.log"
            fake_sweep, fake_remote = write_fake_suite_scripts(tmp_path)
            ...
            result = subprocess.run(
                ...
                encoding="utf-8",
                env=isolated_remote_env(
                    JETSON_TENCENT_TEXT_RUN_PREFIX="tencent-text-ranking-fail-unit",
                    JETSON_TENCENT_TEXT_FAIL_ON_RANKING_PRECHECK="1",
                    JETSON_REMOTE_SYNC="0",
                    JETSON_REMOTE_SWEEP=str(fake_sweep),
                    JETSON_REMOTE_EXEC=str(fake_remote),
                    FAKE_SUITE_LOG=str(log_file),
