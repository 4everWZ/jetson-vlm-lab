...
MiniCPM-V 4.6 当前按 llama.cpp 本地转换路线管理：

```bash
scripts/wsl/prepare_minicpmv46_q4.sh
```

这个脚本会下载官方 HF checkpoint 子集，转换 language GGUF 和 mmproj GGUF，再把 language GGUF 量化为 Q4_K_M。只有当该命令在当前机器上跑完，并且真实 `llama-server` 请求成功后，才算可用。
