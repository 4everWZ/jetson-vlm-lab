...
lane, the first candidate in each existing bundle order is the lane `primary`,
and empty lanes stay explicit. This is a consumer/export layer only; it does
not rerank rows or redefine `Ranking precheck` / `Promotion precheck`.
