...
- Jetson: Docker and NVIDIA runtime for the self-built official llama.cpp image used by the observed multimodal smoke paths, or an equivalent local llama.cpp build.
...
- Jetson container selection: default to the self-built official llama.cpp image; use `LLAMA_CPP_DOCKER_IMAGE=...` for an explicit image, or `LLAMA_CPP_USE_AUTOTAG=1` only for scoped `autotag llama_cpp` experiments.

## Known Hard Boundaries

- No performance claim is valid until a real model/server run completes.
- Jetson support claims are limited to the observed MiniCPM-V 4.6 Q4 and Gemma Q4 smoke paths until additional Jetson logs exist.
