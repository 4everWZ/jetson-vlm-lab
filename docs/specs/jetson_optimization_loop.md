...
`python -m edge_vlm.optimization report` ranks benchmark JSONL files only after
checking:
...
The plan records server commands, benchmark output paths, environment overrides,
and fake-stream commands. The sweep sets `DOCKER_TTY=0` so Docker can run under
background automation instead of requiring an interactive terminal.
...
5. the exact server parameters and Jetson memory notes are documented in a
