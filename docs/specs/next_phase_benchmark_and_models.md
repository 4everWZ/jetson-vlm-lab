...
| Youtu-LLM 2B Q8_0 | `configs/models/tencent_youtu_llm_2b_q8.yaml` | `tencent/Youtu-LLM-2B-GGUF` / `Youtu-LLM-2B-Q8_0.gguf`; strict prepared repeat `tencent-text-repeat5-prepctx-20260609T131412Z` passed 20/20 with `Promotion precheck = yes`, and cached rerun `youtu-llm-q8-cached-20260609T133339Z` recorded `artifact_check_or_download = cached (0.002 s)`, `Startup s = 5.015`, and `Text tok/s = 24.621`; F16 sibling is deferred from the default suite due higher memory pressure |
