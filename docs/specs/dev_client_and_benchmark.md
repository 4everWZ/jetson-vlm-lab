...
- `edge_vlm.fake_stream.run_fake_stream`: Iterates a sorted image folder on a fixed-cadence schedule and writes one response record per frame, including per-frame `input_timing` and `stream_timing` when available.
