...
| R12 | Validate without overstating runtime support | In progress | Tests, syntax checks, dry-run paths, Gemma Q8 artifact checks | Final validation commands and Gemma Q8 text smoke | Gemma Q8 model/mmproj downloaded and text-only smoke passed. MiniCPM conversion, image requests, Jetson runtime, and performance are still unverified. |
